...
import MapMatchers "../src/matchers/Map";
...
let map : Map.Map<Text, Nat> = Map.empty();
Map.add(map, Text.compare, "key1", 20);
Map.add(map, Text.compare, "key2", 10);
...
let suite = Suite.suite(
  ...
  [
    ...
    Suite.suite(
      "Hashmap matchers",
      [
        Suite.test("Should have key", map, MapMatchers.hasKey<Text, Nat>(Text.compare, T.text("key1"))),
        Suite.test(
          "Should fail with missing key",
          map,
          shouldFailWith(
            MapMatchers.hasKey<Text, Nat>(Text.compare, T.text("unknown")),
            "Missing key \"unknown\"",
          ),
        ),
        Suite.test("Should match at key", map, MapMatchers.atKey<Text, Nat>(Text.compare, T.text("key1"), equals20)),
        Suite.test(
          "should fail at key",
          map,
          shouldFailWith(MapMatchers.atKey<Text, Nat>(Text.compare, T.text("key2"), equals20), "10 was expected to be 20"),
        ),
        Suite.test(
          "Should fail with missing key2",
          map,
          shouldFailWith(
            MapMatchers.atKey<Text, Nat>(Text.compare, T.text("unknown"), equals20),
            "Missing key \"unknown\"",
          ),
        ),
      ],
